...
def _get_custom_plain_logger():
    from .custom_logger import LOW_PRI_LOGGER_HEADER_COLOR_PREFIX as header_prefix, INFO_ID, RESET
    return logging.Formatter(
        "{header}%(asctime)s - {child_name} - %(module)s.%(funcName)s(...) - %(lineno)d:{reset}"
        "\n\t%(message)s\n".format(
            header=header_prefix.format(INFO_ID),
            child_name=__name__+": plain logger",
            reset=RESET),
        "%H:%M:%S")


plain_info_logger = get_logger("EmailClassifier",__name__+": plain logger",
                                   formatter=_get_custom_plain_logger(),level="INFO")
