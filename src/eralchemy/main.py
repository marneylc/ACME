...
from src.eralchemy.version import version as __version__
from src.eralchemy.cst import GRAPH_BEGINNING
from src.eralchemy.sqla import metadata_to_intermediary, declarative_to_intermediary, database_to_intermediary
from src.eralchemy.helpers import check_args
from src.eralchemy.parser import markdown_file_to_intermediary, line_iterator_to_intermediary, ParsingException
...
def filter_resources(tables, relationships,
    ...
    """
    def convert_to_set(target,default_data=None):
        if target is not None:
            ...
            if default_data is None:
                ...
                target = set(default_data)
        return target

    _tables = copy.deepcopy(tables)
    _relationships = copy.deepcopy(relationships)

    exclude_tables = convert_to_set(exclude_tables)
    include_tables = convert_to_set(include_tables, (t.name for t in _tables if t.name not in exclude_tables))
    exclude_columns = convert_to_set(exclude_columns)
    include_columns = convert_to_set(include_columns, (c.name for t in _tables for c in t.columns if c.name not in exclude_columns))

    _tables = [t for t in _tables
               if t.name in include_tables]
    _relationships = [r for r in _relationships
                      if r.right_col in include_tables and r.left_col in include_tables]

    for t in _tables:
        t.columns = [c for c in t.columns if c.name in include_columns]
...
def render_er(input, output, mode='auto', include_tables=None, include_columns=None,
    ...
    if isinstance(output,str):
        output = [output]
    try:
        _tables, _relationships = all_to_intermediary(input, schema=schema)
    ...
    for out in output:
        try:
            tables, relationships = filter_resources(_tables, _relationships,
            ...
            intermediary_to_output = get_output_mode(out, mode)
            intermediary_to_output(tables, relationships, out)
        except ImportError as e:
            module_name = e.message.split()[-1]
            print('Please install {0} using "pip install {0}".'.format(module_name))
        except ParsingException as e:
